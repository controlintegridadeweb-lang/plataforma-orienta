...
const v2Payload = z.object({
  ...
  answer: z.enum(["yes", "no", "not_applicable"]).optional(),
  ...
  /** Metrica de texto livre: cenario definido pela validacao da evidencia. */
...
export async function POST(request: Request) {
    ...
    return NextResponse.json(
      {
        ...
        message: "Este endpoint aceita apenas payload V2 com useSnapshot=true.",
      },
      { status: 400 },
    );
  }
  try {
    ...
    let mappedAnswer: "yes" | "no" | "not_applicable" | undefined = parsed.data.answer;
